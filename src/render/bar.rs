...
pub async fn draw_single_bar(x_level: f32, ys: &[f64], bar: &Bar) {
...
pub async fn run(bar: Bar, _min_y: f64) {
    ...
    if min_y >= 0. {
        positive_only(bar, step_y, y_half_font, y_font_size, max_y, ys).await;
    } else {
...
pub async fn positive_negative(bar: Bar, step_y: f64, y_half_font: f32, y_font_size: f32, max_y: f64, ys: Vec<f64>) {
    loop {
        ...
        draw_single_bar(screen_height() / 2., &ys, &bar).await;
...
pub async fn positive_only(bar: Bar, step_y: f64, y_half_font: f32, y_font_size: f32, max_y: f64, ys: Vec<f64>) {
    loop {
        ...
        draw_single_bar(x_level, &ys, &bar).await;
