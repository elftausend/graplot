...
thread_local! {
    static MAIN_FUTURE1: RefCell<Option<Pin<Box<dyn Future<Output = ()>>>>> = RefCell::new(None);
    #[no_mangle]
...
//fn get_context1() -> &'static mut Context {
//    unsafe { CONTEXT.as_mut().unwrap_or_else(|| panic!()) }
//}

fn get_context() -> &'static mut Context {
    let ctx = CONTEXT.with(|ctx: & _| {
        let mut c = ctx.borrow_mut();
        let d = c.as_mut().unwrap();
        
        let a = d as *mut Context;
        a
    });
    unsafe {&mut *ctx}
    ...
    fn draw(&mut self) {
        {
            ...
            let result = maybe_unwind(get_context().unwind, || {
                ...
                MAIN_FUTURE1.with(|mf| {
                    let mut a = mf.borrow_mut();
                    let c = a.as_mut();
                    if let Some(future) = c {
            ...
            if result == false {
                if let Some(recovery_future) = get_context().recovery_future.take() {
                    MAIN_FUTURE1.with(|mf| *mf.borrow_mut() = Some(recovery_future));
                }
            }
...
impl Window {
    ...
    pub fn from_config(config: conf::Conf, future: impl Future<Output = ()> + 'static) {
        miniquad::start(
            ...
            |ctx| {
                MAIN_FUTURE1.with(|mf| *mf.borrow_mut() = Some(Box::pin(future)));
                
                CONTEXT.with(|sctx| *sctx.borrow_mut() = Some(Context::new(ctx)));
                UserData::free(Stage {})
            },
        );
    }
